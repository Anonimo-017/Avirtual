...
function getActivityLogData($con, $searchKeyword = '')
{
    $query = "SELECT * FROM activity_log";
    if (!empty($searchKeyword)) {
        $query .= " WHERE username LIKE '%" . mysqli_real_escape_string($con, $searchKeyword) . "%' OR action LIKE '%" . mysqli_real_escape_string($con, $searchKeyword) . "%'";
    }
...
$activityLogData = getActivityLogData($con, $searchKeyword);
...
$pdf->SetSubject('Activity Log Report');
$pdf->SetKeywords('PDF, report, activity log');
...
if (!empty($searchKeyword)) {
    $pdf->Write(0, 'Reporte de actividades de: ' . htmlspecialchars($searchKeyword, ENT_QUOTES, 'UTF-8') . "\n\n");
}

$activityLogHeaders = ['Fecha', 'Usuario', 'Acción'];
$activityLogDataFormatted = array_map(function ($row) {
    return [
        $row['date'],
        $row['username'],
        $row['action']
...
generateTable($pdf, $activityLogDataFormatted, $activityLogHeaders);
$pdf->Output('reporte_actividades.pdf', 'D');